...
def cycle_calc(input):
    ...
    for j in range(9999):
        ...
        if balance < cycle_tolerance:
            break
    KPD = 0.99*0.99*(read_block('T')["Q"] - read_block('MC')["Q"] - read_block('RC')["Q"] ) / read_block('RHE')["Q"]
    print(P, p_out, x, KPD)
    close_db()
    return -KPD
...
res = minimize(cycle_calc, (25, 8, 0.65), method='Nelder-Mead',tol=1e-4, bounds=((20, 30), (7.7, 9), (0.6, 0.75)))
